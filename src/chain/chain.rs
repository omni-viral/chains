...
use chain::raw::PassLinks;
use chain::link::{Acquire, ChainLink, LinkSync, Release};
...
pub struct Chain<A, L, U, S, W> {
    usage: U,
    links: Vec<Option<ChainLink<A, L, S, W>>>,
}
    ...
    U: Usage,
{
    ...
    fn build<P, R, F>(id: ChainId<R>, mut usage: U, passes: P, mut new_semaphore: F) -> Self
    where
        P: IntoIterator,
        ...
        F: FnMut() -> (S, W),
    {
        let mut links: Vec<Option<ChainLink<A, L, S, W>>> = Vec::new();
...
impl<A, L, U, S, W> Chain<A, L, U, S, W> {
    ...
    ///
    pub fn link(&self, index: usize) -> &ChainLink<A, L, S, W> {
    ...
    /// This function will panic if no link exists at this index.
    ///
    pub fn link_mut(&mut self, index: usize) -> &mut ChainLink<A, L, S, W> {
        self.links[index].as_mut().unwrap()
    }

    /// Get reference to first link before specified index.
    pub fn prev(&self, index: usize) -> Option<&ChainLink<A, L, S, W>> {
        self.links[..index]
            .iter()
            .rev()
    ...
    pub fn prev_mut(&mut self, index: usize) -> Option<&mut ChainLink<A, L, S, W>> {
        self.links[..index]
            .iter_mut()
            .rev()
            .filter_map(Option::as_mut)
    ...
    pub fn next(&self, index: usize) -> Option<&ChainLink<A, L, S, W>> {
        self.links[index + 1..]
            .iter()
            .filter_map(Option::as_ref)
            .next()
    ...
    pub fn next_mut(&mut self, index: usize) -> Option<&mut ChainLink<A, L, S, W>> {
        self.links[index + 1..]
            .iter_mut()
            .filter_map(Option::as_mut)
            .next()
    ...
    pub fn first(&self) -> &ChainLink<A, L, S, W> {
    ...
    ///
    /// This function will panic if no link exists.
    pub fn first_mut(&mut self) -> &mut ChainLink<A, L, S, W> {
        self.links
            .iter_mut()
            .filter_map(Option::as_mut)
            .next()
    ...
    ///
    /// This function will panic if no link exists.
    pub fn last(&self) -> &ChainLink<A, L, S, W> {
        self.links
            .iter()
            .rev()
            .filter_map(Option::as_ref)
    ...
    ///
    /// This function will panic if no link exists.
    pub fn last_mut(&mut self) -> &mut ChainLink<A, L, S, W> {
        self.links
            .iter_mut()
            .rev()
            .filter_map(Option::as_mut)
...
pub struct Chains<A, L, U, S, W> {
    chains: Vec<Chain<A, L, U, S, W>>,
}

impl<A, L, U, S, W> Chains<A, L, U, S, W>
where
    A: Access,
    L: Layout,
    U: Usage,
{
    /// Create new
    pub fn new<R>(count: usize, usage: U, passes: &[PassLinks<R>]) -> Chains<A, L, U, S, W>
    where
        R: Resource<Access = A, Layout = L, Usage = U>,
    {
        Chains {
            chains: (0..count)
                .map(|i| Chain::build(ChainId::new(i), usage, passes, || unimplemented!()))
                .collect(),
        }
    }
...
impl<R, A, L, U, S, W> Index<ChainId<R>> for Chains<A, L, U, S, W>
where
    A: Access,
    L: Layout,
    U: Usage,
...
impl<R, A, L, U, S, W> IndexMut<ChainId<R>> for Chains<A, L, U, S, W>
where
    A: Access,
    L: Layout,
    U: Usage,
